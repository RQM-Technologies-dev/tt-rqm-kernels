# Documentation index

The root README provides the five-minute project path. This page keeps the
deeper technical, operational, historical, and strategy material discoverable
without making it part of the primary introduction.

## Architecture and operator contracts

- [Operator contracts](operator-contracts.md)
- [StructuredBench specification](structuredbench-spec.md)
- [TT-Metalium qmul design](tt-metalium-qmul-design.md)
- [SU2HamiltonianBench roadmap](hamiltonian-evolution-roadmap.md)
- [H2A TT-Metalium single-core candidate](../experimental/tt_metalium_hamiltonian_lowering/README.md)
- [H2A compensated Candidate B](../experimental/tt_metalium_hamiltonian_lowering_compensated/README.md)
- [H2B two-program TT-Metal candidate](../experimental/tt_metalium_hamiltonian_evolution/README.md)
- [EntanglementDynamicsBench reference foundation](benchmarks/entanglement-dynamics-bench.md)
- [Complex/quaternion bridge](complex-quaternion-bridge.md)
- [Phase-update plan](phase-update-tenstorrent-plan.md)

## Benchmark evidence and methodology

- [Benchmark reports](benchmarks/index.md)
- [Claim policy](benchmarks/claim-policy.md)
- [SU2ComposeBench claim policy](benchmarks/su2-compose-claim-policy.md)
- [SU2ComposeBench stability methodology](su2-stability-methodology.md)
- [Retained SU2 candidate experiment](benchmarks/su2-compose-candidate-54b91b.md)
- [Retained SU2 v2 qualification result](../benchmarks/processed/wormhole-su2-compose-stability-qualification.json)
- [H2A pre-hardware preregistration](../benchmarks/manifests/hamiltonian-lowering-h2a-preregistration.json)
- [H2A retained N300 development blocker](../benchmarks/pilots/hamiltonian-lowering-h2a/h2a-n300-development-blocker-20260716/README.md)
- [H2A compensated development comparison](../benchmarks/pilots/hamiltonian-lowering-h2a/h2a-compensated-development-20260716/README.md)
- [H2A non-designated pilot](../benchmarks/pilots/hamiltonian-lowering-h2a/h2a-compensated-n300-pilot-20260716/suite-report.md)
- [H2A Claim Level 0 silicon-conformance report](benchmarks/hamiltonian-lowering-h2a.md)
- [H2B retained failed Contract-v1 N300 sessions](benchmarks/hamiltonian-evolution-h2b.md)
- [Stage B stability methodology](stage-b-stability-methodology.md)
- [Tenstorrent execution runbook](tenstorrent-execution-runbook.md)
- [Functional quickstart](tenstorrent-functional-quickstart.md)

## Simulator and emulation

- [TT-Lang qmul plan](tt-lang-qmul-plan.md)
- [tt-emule qmul validation](tt-emule-qmul-validation-plan.md)
- [Structured qmul tutorial](structured-qmul-tutorial.md)

## Hardware access and handoff history

- [Console access plan](tenstorrent-console-access-plan.md)
- [Console copy/paste instructions](tenstorrent-console-copy-paste.md)
- [Cloud-access plan](tenstorrent-cloud-access-plan.md)
- [Delegated validation](tenstorrent-delegated-validation.md)
- [Engineer copy/paste packet](tenstorrent-engineer-copy-paste-packet.md)
- [Hardware command contract](tenstorrent-hardware-command-contract.md)

These pages preserve setup and access history. Approved N300 execution has
already produced the committed Stage A, Stage B, persistent qmul, and H1 SU(2)
evidence. The retained v2 SU2 campaign did not qualify, while the separate
fused-only v3 campaign established the public Claim Level 2 release.

## Integration and contribution paths

- [External contribution selection](external-tenstorrent-contribution-selection.md)
- [TT-MLIR fused-lowering prerequisites](tt-mlir-fused-lowering-prerequisites.md)
- [Tenstorrent RFC](tenstorrent-rfc.md)
- [Collaboration map](collaboration-map.md)

## Longer-term research directions

- [QuantumIR](quantum-ir.md)
- [QuantumIR roadmap](quantum-ir-roadmap.md)
- [QuantumIR operator mapping](quantum-ir-operator-mapping.md)
- [Physical-AI state integrity](future-physical-ai-state-integrity.md)
- [Physical-AI pose-stream demo](physical-ai-pose-stream-demo.md)
- [StructuredBench HPC expansion](structuredbench-hpc-expansion-roadmap.md)

These are future directions, not current hardware, stability, acceleration, or
application claims.
