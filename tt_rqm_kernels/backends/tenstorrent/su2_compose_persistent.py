...
from tt_rqm_kernels.benchmark_integrity import IntegrityError, command_sha256, repository_commit, timing_summary
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="tt-rqm-su2-compose-") as temp:
        ...
        for batch, steps, repeats, warmups, samples in specs:
            ...
            case = {
                "case_id": case_id, "B": batch, "K": steps,
                "repeat_count": repeats, "warmup_pairs": warmups, "samples": samples,
                "inputs": {"rotors": rotor_name, "phases": phase_name,
                           "rotors_sha256": rotor_hash, "phases_sha256": phase_hash},
                "outputs": outputs,
            }
            cases.append(case)
        ...
        manifest = {
            "schema": PROTOCOL, "workload": "su2_compose", "dtype": "float32",
            "device_id": 0, "seed": seed, "cases": cases, "outputs": {"metrics": "metrics.json"},
        }
        manifest_path = workdir / "manifest.json"
        manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True) + "\n")
        host_start = time.perf_counter()
        _run_candidate(command, workdir, manifest_path, candidate_hash, source_commit)
        ...
        for (case, coefficients, rotors, phases), candidate_case in zip(
            ...
            fused_correctness = _correctness(fused_rotor, fused_phase, reference_rotor, reference_phase)
            unfused_correctness = _correctness(unfused_rotor, unfused_phase, reference_rotor, reference_phase)
            oracle_error, end_to_end_error = _oracle_errors(
                coefficients, fused_rotor, fused_phase
            )
            if oracle_error > 1e-11:
                raise IntegrityError(f"CPU oracles disagree for B={batch}, K={steps}: {oracle_error}")
            ...
            results.append({
                "case_id": case["case_id"], "B": batch, "K": steps,
                "repeat_count": repeats, "warmup_pairs": case["warmup_pairs"], "samples": case["samples"],
                "stable_benchmark": False, "performance_eligible": normalized["performance_eligible"],
                ...
                "comparison": {
                    "fused_over_unfused_median": float(fused_summary["median"]) / float(unfused_summary["median"]),
    ...
    report: dict[str, object] = {
        "schema": REPORT_SCHEMA, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "benchmark": "SU2ComposeBench", "family": "SU2HamiltonianBench", "benchmark_stage": stage,
        "protocol": PROTOCOL, "device": DEVICE, "dtype": "float32", "seed": seed,
        "execution_label": "hardware", "stable_benchmark": False,
        "performance_eligible": normalized["performance_eligible"], "methodology_note": methodology_note,
        "python_version": sys.version.split()[0], "platform": platform.platform(),
        "lifecycle": normalized["lifecycle"], "session_timing": {**normalized["session_timings_s"], "host_process_end_to_end": host_s},
        "provenance": {"repository_commit": source_commit, "candidate_sha256": candidate_hash,
                       "candidate": normalized["provenance"]},
        "results": results,
        "nonclaims": ["no_stability_claim", "no_acceleration_claim", "no_cpu_comparison",
                      "no_measured_bandwidth_claim", "no_full_device_side_hamiltonian_lowering_claim"],
    }
    return report


def _correctness(
    rotor: torch.Tensor, phase: torch.Tensor, reference_rotor: torch.Tensor, reference_phase: torch.Tensor
) -> dict[str, object]:
    ...
    return {
        "validated_values": int(actual.numel()), "failing_values": failures,
        ...
        "quaternion_norm_drift": float((torch.linalg.vector_norm(rotor.double(), dim=-1) - 1).abs().max().item()),
        "phase_norm_drift": float((torch.linalg.vector_norm(phase.double(), dim=-1) - 1).abs().max().item()),
        "unitarity_frobenius_error": float(torch.linalg.matrix_norm(matrix.mH @ matrix - identity).max().item()),
        "su2_determinant_error": float((torch.linalg.det(su2) - 1).abs().max().item()),
        "u2_phase_consistency_error": float((torch.linalg.det(matrix) - phase_complex.square()).abs().max().item()),
        "bloch_norm_drift": float((torch.sqrt(x.square() + y.square() + z.square()) - 1).abs().max().item()),
    }
...
def _oracle_errors(
    ...
    for start in range(0, coefficients.shape[0], 256):
        ...
        hardware = u2_matrix_from_rotor_phase(hardware_rotor[start:stop].double(), hardware_phase[start:stop].double())
        oracle_error = max(oracle_error, float((matrix_oracle - quaternion_oracle).abs().max().item()))
        end_to_end_error = max(end_to_end_error, float((matrix_oracle - hardware).abs().max().item()))
...
def validate_su2_metrics(
    ...
    expected = {
        "schema": METRICS_SCHEMA, "protocol": PROTOCOL, "device": DEVICE, "dtype": "float32",
        "execution_kind": "hardware", "implementation_class": IMPLEMENTATION, "stable_benchmark": False,
    }
    ...
    if metrics.get("lifecycle") != {"device_count": 1, "device_id": 0, "create_count": 1, "close_count": 1}:
    ...
    for key in ("chip_type", "tt_metal_commit", "compiler_version", "runtime_version", "build_id", "repository_commit"):
    ...
    if not isinstance(session, dict) or float(session.get("candidate_session", 0)) <= 0 or float(session["candidate_session"]) > host_s * 1.05 + 1e-6:
        raise IntegrityError("invalid candidate session timing")
    expected_cases, actual_cases = manifest.get("cases"), metrics.get("cases")
    if not isinstance(expected_cases, list) or not isinstance(actual_cases, list) or len(expected_cases) != len(actual_cases):
    ...
    for expected_case, actual in zip(expected_cases, actual_cases, strict=True):
        ...
        for key in ("fused_samples", "unfused_samples", "paired_order"):
            if not isinstance(timings.get(key), list) or len(timings[key]) != int(expected_case["samples"]):
        ...
        if work.get("core_count") != min(tiles, available) or work.get("device_count") != 1 or work.get("device_id") != 0:
            raise IntegrityError("case core/device metadata mismatch")
        if work.get("fused_dispatches_per_chain") != 1 or work.get("unfused_dispatches_per_chain") != int(expected_case["K"]) - 1:
...
def render_su2_markdown(report: Mapping[str, object]) -> str:
    ...
    lines.extend(["", "This H1 report does not claim stability, acceleration, CPU superiority, measured bandwidth, or full device-side Hamiltonian lowering.", ""])
    return "\n".join(lines)


def _run_candidate(command: str, workdir: Path, manifest: Path, candidate_hash: str, source_commit: str) -> None:
    ...
    env = os.environ.copy()
    env.update({
        "TT_RQM_SU2_COMPOSE_DIR": str(workdir), "TT_RQM_CANDIDATE_SHA256": candidate_hash,
        "TT_RQM_BUILD_ID": candidate_hash, "TT_RQM_REPOSITORY_COMMIT": source_commit,
    })
    completed = subprocess.run([*tokens, "--workdir", str(workdir), "--manifest", str(manifest), "--device", "0"], capture_output=True, text=True, env=env)
    if completed.returncode:
        raise IntegrityError(f"SU2ComposeBench candidate failed\nstdout:\n{completed.stdout}\nstderr:\n{completed.stderr}")
...
def _read_float32(path: Path, shape: tuple[int, int]) -> torch.Tensor:
    ...
    if sys.byteorder != "little": values.byteswap()
    if len(values) != shape[0] * shape[1]: raise IntegrityError(f"output length mismatch for {path.name}")
