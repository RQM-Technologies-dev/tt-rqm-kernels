#!/usr/bin/env python3
"""Build the persistent SU2ComposeBench TT-Metalium candidate."""
...
TARGET = "tt_rqm_metalium_su2_compose_candidate"
...
def _package_dir(prefix: Path) -> Path | None:
    ...
    for candidate in candidates:
        if any((candidate / name).exists() for name in ("tt-metalium-config.cmake", "TT-MetaliumConfig.cmake")):
            return candidate
    return None
...
def main() -> int:
    ...
    prefixes = [args.cmake_prefix_path] if args.cmake_prefix_path else [
        root / "build_Release", root / "build", root / ".build" / "default"
    ]
    prefix = next((path.resolve() for path in prefixes if path and _package_dir(path.resolve())), None)
    if prefix is None:
    ...
    command = [
        cmake, "-S", str(PACKAGE), "-B", str(build), "-G", "Ninja",
        f"-DCMAKE_PREFIX_PATH={prefix}", f"-DTT-Metalium_DIR={package_dir}",
    ]
