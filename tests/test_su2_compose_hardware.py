...
def test_hardware_package_contains_separate_fused_and_unfused_paths() -> None:
    required = (
        "src/su2_compose_candidate.cpp",
        "kernels/su2_fused_reader.cpp",
        "kernels/su2_fused_compute.cpp",
        "kernels/su2_fused_writer.cpp",
        "kernels/su2_unfused_reader.cpp",
        "kernels/su2_unfused_compute.cpp",
        "kernels/su2_unfused_writer.cpp",
        "kernels/su2_sfpu.h",
    )
...
def test_preregistered_case_specs_are_exact() -> None:
    ...
    repeat_counts = {
        (batch, steps): index + 1
        for index, (batch, steps) in enumerate(
            (
                (32768, 8),
                (8192, 32),
                (2048, 128),
...
def test_strict_metrics_accept_fused_only_stability_surface() -> None:
    ...
    assert (
        validate_su2_metrics(metrics, manifest, "c" * 64, 3.0)["benchmark_mode"] == FUSED_STABILITY
    )
...
def test_public_hardware_claim_is_immediately_qualified() -> None:
    for path in (
        Path("README.md"),
        Path("docs/tenstorrent-landing.md"),
        Path("docs/benchmarks/su2-compose-bench.md"),
    ):
        text = path.read_text()
...
def test_readme_summarizes_all_public_benchmark_releases() -> None:
